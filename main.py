...
def get_accuracy(t, p):
    ...
    return (t == pred).long().sum().item() / p.size(0)
...
def valid(model, valid_loader, valid_loss, valid_accu, loss_function):
    model.eval()
...
def train(model, opt, loss_function, train_dataset, valid_dataset, is_print_size=False, bert=False):
    ...
    while epoch:
        ...
        epoch -= 1
        _valid_loss, _valid_accu = valid(model, valid_loader, valid_loss, valid_accu, loss_function)
